...
Here, each segment corresponds to a segment of the (final) PackedMarkerGraph.
During detangling, segments are replicated, and therefore
there can be multiple AssemblyGraph segments
corresponding to a single PackedMarkerGraph segment.
For each of the replcas we store which oriented reads are
believed to "belong" to that copy.
...
Each segment replica is a vertex and each link is an edge
(contrary to what happens in the PackedMarkerGraph).

Connectivity in the AssemblyGraph is generated by "following the reads":
a link s0->s1 between segments replicas s0 and s1 is created if an oriented
